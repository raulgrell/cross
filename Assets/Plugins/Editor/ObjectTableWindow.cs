            ...
            if (_selectedIndex != -1 && _objectTypeInfos[_selectedIndex].isScriptableObject)
            {
                ...
                if (GUI.Button(controlRect, "New"))
                {
                    ...
                    string path = AssetDatabase.GenerateUniqueAssetPath(
                        "Assets" + _targetFolderForNewAsset + "/new" + _objectTypeNames[_selectedIndex] + ".asset");
                ...
                if (GUI.Button(controlRect, "Pick Folder"))
                {
                    ...
                    string value = EditorUtility.SaveFolderPanel("Pick default folder for new asset", fullCurrentPath,
                        "database");
                    if (value.Length != 0)
                    {
                        if (!value.Contains(Application.dataPath))
                        {
                            Debug.LogErrorFormat(
                                "Picked folder {0} isn't in the Assets folder. Pick a folder in the Assets folder",
                                value);
                        }
                        else
                ...
                EditorGUI.SelectableLabel(controlRect,
                    _targetFolderForNewAsset == ""
                        ? "None, Pick a folder where new assets will be created"
                        : _targetFolderForNewAsset);
            }
...
public class DatabaseDisplayer : TreeView
{
    ...
    public DatabaseDisplayer(TreeViewState state, MultiColumnHeader header, ObjectTable.TypeInfo objectType) :
        base(state, header)
    {
        _freeID = 0;
    ...
    protected override void KeyEvent()
    {
        if (Event.current.type == EventType.KeyUp && Event.current.keyCode == KeyCode.Delete)
        {
            ...
            if (EditorUtility.DisplayDialog("Confirm",
                "Confirm the suppression of the " + list.Count + " elected element?\nThis can't be undone.", "Yes",
                "No"))
            {
                var rows = GetRows();
    ...
    protected override TreeViewItem BuildRoot()
    {
        ...
        if (_objectType.isScriptableObject)
        {
            ...
            objs = new UnityEngine.Object[assets.Length];
        ...
        else
        {
            ...
            for (int i = 0; i < assets.Length; ++i)
            {
                UnityEngine.Object obj = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(assets[i]), _objectType.type);
                if (obj != null)
