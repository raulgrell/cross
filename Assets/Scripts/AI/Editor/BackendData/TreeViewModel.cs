...
internal class TreeViewModel<T> : TreeView where T : TreeElement
{
    ...
    public TreeViewModel(TreeViewState state, TreeModel<T> model) : base(state)
    {
        Init(model);
    }

    public TreeViewModel(TreeViewState state, MultiColumnHeader multiColumnHeader, TreeModel<T> model)
