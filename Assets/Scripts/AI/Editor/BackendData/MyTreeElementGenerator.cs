...
static class MyTreeElementGenerator
{
    ...
    static int minNumChildren = 1;
    static int maxNumChildren = 3;
    static float probabilityOfBeingLeaf = 0.5f;

    public static List<TreeTask> GenerateRandomTree(int numTotalElements)
    {
        int numRootChildren = numTotalElements / 4;
        IDCounter = 0;
        var treeElements = new List<TreeTask>(numTotalElements);

        var root = new TreeTask("Root", -1, IDCounter);
    ...
    static void AddChildrenRecursive(TreeElement element, int numChildren, bool force, int numTotalElements,
        ref int allowedDepth, List<TreeTask> treeElements)
    {
        ...
        for (int i = 0; i < numChildren; ++i)
        {
            ...
            var child = new TreeTask("Element " + IDCounter, element.depth + 1, ++IDCounter);
