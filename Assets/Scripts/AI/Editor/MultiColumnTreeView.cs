...
internal class MultiColumnTreeView : TreeViewModel<TreeTask>
{
    ...
    public MultiColumnTreeView(TreeViewState state, MultiColumnHeader multicolumnHeader, TreeModel<TreeTask> model) :
    ...
    void SortByMultipleColumns()
    {
        ...
        var myTypes = rootItem.children.Cast<TreeViewItem<TreeTask>>();
        ...
        for (int i = 1; i < sortedColumns.Length; i++)
        {
            ...
            switch (sortOption)
            {
                ...
                case SortOption.Value2:
                    orderedQuery = orderedQuery.ThenBy(l => l.data.status, ascending);
                    break;
    ...
    IOrderedEnumerable<TreeViewItem<TreeTask>> InitialOrder(IEnumerable<TreeViewItem<TreeTask>> myTypes, int[] history)
    {
        ...
        switch (sortOption)
        {
            ...
            case SortOption.Value2:
                return myTypes.Order(l => l.data.status, ascending);
            case SortOption.Value3:
                return myTypes.Order(l => l.data.status, ascending);
            default:
    ...
    int GetIcon1Index(TreeViewItem<TreeTask> item)
    {
        return (int) (Mathf.Min(0.99f, (float) item.data.status) * s_TestIcons.Length);
    }
    ...
    void CellGUI(Rect cellRect, TreeViewItem<TreeTask> item, MyColumns column, ref RowGUIArgs args)
    {
            ...
            case MyColumns.Value3:
            {
                if (showControls)
                {
                    ...
                    if (column == MyColumns.Value2)
                        item.data.status = (TaskStatus) EditorGUI.EnumPopup(cellRect, item.data.status);
                ...
                else
                {
                    ...
                    if (column == MyColumns.Value2)
                        value = item.data.status.ToString();
                    if (column == MyColumns.Value3)
                        value = item.data.status.ToString();
    ...
    public static MultiColumnHeaderState CreateDefaultMultiColumnHeaderState(float treeViewWidth)
    {
        var columns = new[]
        {
            ...
            new MultiColumnHeaderState.Column
            {
                headerContent = new GUIContent("Status", "Current task status"),
            ...
            new MultiColumnHeaderState.Column
            {
                headerContent = new GUIContent("Note", "Task details"),
                headerTextAlignment = TextAlignment.Right,
                sortedAscending = true,
                sortingArrowAlignment = TextAlignment.Left,
                width = 70,
