...
public class TargetDrawer : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        var positionRect = new Rect(position.x, position.y, position.width / 3 - 5, position.height);
        var effectRect = new Rect(position.x + position.width / 3 + 5, position.y, position.width / 3 - 5, position.height);
        var damageRect = new Rect(position.x + 2 * position.width / 3 + 10, position.y, position.width / 3 - 10, position.height);
